...
class CompanyScraper:
    ...
    def scrape_company(self, company_url):
        if not self._is_valid_company_url(company_url):
            print(f"[ERROR] Invalid company URL: {company_url}")
            return None
        ...
        try:
            ...
            if not self._wait_for_company_page():
                print(f"[ERROR] Failed to load company page: {company_url}")
                return None
            ...
            print(f"[SUCCESS] Successfully scraped company: {company_data.get('name', 'Unknown')}")
            print(f"   Industry: {company_data.get('industry', 'N/A')}")
            print(f"   Location: {company_data.get('location', 'N/A')}")
            print(f"   Employee Count: {company_data.get('employee_count', 'N/A')}")
            
            return company_data
            
        except Exception as e:
            print(f"[ERROR] Error scraping company {company_url}: {str(e)}")
            return None
    ...
    def _extract_verification_status(self):
        try:
            verified_selectors = [
            ...
            for selector in selectors:
                try:
                    ...
                    for element in elements:
                        ...
                        if text and ('Technology' in text or 'Information' in text or 
                                   'Internet' in text or 'Software' in text or
                                   'Finance' in text or 'Healthcare' in text or
                                   'Manufacturing' in text or 'Education' in text or
                                   'Retail' in text or 'Services' in text):
                            return text
                except Exception:
                    continue
        ...
        try:
            ...
            for selector in selectors:
                try:
                    ...
                    for element in elements:
                        text = element.text.strip()
                        if text and self._is_likely_location(text):
                            return text
                except Exception:
                    continue
    ...
    def _is_likely_location(self, text):
        ...
        common_locations = [
            'United States', 'USA', 'Canada', 'UK', 'United Kingdom', 'India', 'Australia',
            'Germany', 'France', 'Japan', 'China', 'Brazil', 'Mexico', 'Netherlands',
            'Sweden', 'Norway', 'Denmark', 'Finland', 'Switzerland', 'Austria',
            'California', 'New York', 'Texas', 'Florida', 'Illinois', 'Washington',
            'Massachusetts', 'Virginia', 'Georgia', 'Colorado', 'Pennsylvania',
            'San Francisco', 'Los Angeles', 'Chicago', 'Boston', 'Seattle', 'Austin',
            'Denver', 'Atlanta', 'Miami', 'Dallas', 'London', 'Toronto', 'Vancouver',
            'Mumbai', 'Bangalore', 'Delhi', 'Sydney', 'Melbourne', 'Berlin', 'Paris'
        ]
                    ...
                    for element in elements:
                        ...
                        if text and ('followers' in text.lower() or 'M' in text or 'K' in text):
                            if 'followers' in text.lower():
                                return text
                            elif any(char in text for char in ['M', 'K']) and len(text) < 10:
                                return text
                except Exception:
                    continue
    ...
    def _extract_employee_count(self):
        try:
            selectors = [
            ...
            for selector in selectors:
                try:
                    ...
                    for element in elements:
                        text = element.text.strip()
                        if text and ('employee' in text.lower() or 
                                   self._contains_employee_range(text)):
                            return text
                except Exception:
                    continue
    ...
    def _contains_employee_range(self, text):
        import re
        patterns = [
            r'\d+[-–]\d+',
            r'\d+K[-–]\d+K',
    ...
    def _navigate_to_about_page(self):
        try:
            ...
            print(f"Error navigating to about page: {str(e)}")
            return {}
    ...
    def _extract_headquarters(self):
        try:
                        ...
                        if keyword in element.text.lower():
                            try:
                                ...
                                try:
                                    ...
                                    dd_element = parent.find_element(By.XPATH, "./following-sibling::dd")
                                    headquarters = dd_element.text.strip()
                                    if headquarters:
                                        return headquarters
                                except:
            ...
            for selector in fallback_selectors:
                try:
                    ...
                    for element in elements:
                        text = element.text.strip()
                        if text and self._is_likely_location(text) and len(text) > 5:
                            return text
                except Exception:
                    continue
    ...
    def _is_valid_year(self, text):
        import re
        year_pattern = r'\b(19|20)\d{2}\b'
        return bool(re.search(year_pattern, text))

    def _extract_company_size(self):
        try:
            selectors_strategies = [
                ("dt h3", "company size"),
    ...
    def _extract_associated_members(self):
        try:
            selectors = [
    ...
    def _is_member_count(self, text):
        import re
        patterns = [
            r'\d+,\d+',
            r'\d+\.\d+K',
    ...
    def _extract_verified_date(self):
        try:
            selectors_strategies = [
                ("dt h3", "verified page"),
    ...
    def _wait_for_company_page(self):
        try:
            self.wait.until(lambda driver: driver.current_url != "about:blank")
            
            current_url = self.driver.current_url
            if not ("linkedin.com/company/" in current_url):
                return False
